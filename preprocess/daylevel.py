"""daylevel.py — the day-level feature table.

Input   <data>/ch2025_data_items/*.parquet, ch2026_metrics_train.csv, ch2026_submission_sample.csv
Output  cache/features.parquet   700 x (2 keys + sleep_date + split + 7 labels + 281 features);
                                 sub rows carry NaN labels
        cache/feature_column_groups.json   diagnostic; nothing reads it
Used by modeling/features/tabular.py:load_features

Rows are all_keys sorted by (subject_id, lifelog_date); sleep_tensor.py and context.py match it.

Run: python preprocess/daylevel.py     (PREPROCESS_VERBOSE=1 for the diagnostic tables)
"""
